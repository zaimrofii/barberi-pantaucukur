...
class BarberDetector:
    def __init__(self, model_path='yolov8n.pt', rois=None):
        ...
        # --- NEW: Frame counter for skipping ---
        self.frame_count = 0
        self.skip_frames = int(os.environ.get('AI_SKIP_FRAMES', '10'))
        ...
        print("Sistem AI: Model siap dengan ByteTrack, MediaPipe Pose, dan Scoring Engine.")
    ...
    def classify_posture(self, landmarks):
        ...
        try:
            ...
            # If shoulders are significantly higher than hips, person is standing
            # If shoulders and hips are close vertically, person is sitting
    ...
    def calculate_hand_activity(self, landmarks, prev_landmarks=None):
        ...
        try:
            ...
            head_center = (nose.x, nose.y)
            for wrist in [left_wrist, right_wrist]:
                distance = np.sqrt((wrist.x - head_center[0])**2 + (wrist.y - head_center[1])**2)
                if distance < 0.2:
                    points += 1
            
            # 3. Fast hand movement (if previous landmarks available)
            if prev_landmarks is not None:
                prev_left_wrist = prev_landmarks[mp.solutions.pose.PoseLandmark.LEFT_WRIST.value]
                prev_right_wrist = prev_landmarks[mp.solutions.pose.PoseLandmark.RIGHT_WRIST.value]
                
                left_speed = np.sqrt((left_wrist.x - prev_left_wrist.x)**2 + (left_wrist.y - prev_left_wrist.y)**2)
                right_speed = np.sqrt((right_wrist.x - prev_right_wrist.x)**2 + (right_wrist.y - prev_right_wrist.y)**2)
                
                if left_speed > 0.05 or right_speed > 0.05:
                    points += 1
            ...
            for wrist, hip in [(left_wrist, left_hip), (right_wrist, right_hip)]:
                if abs(wrist.y - hip.y) < 0.1:
                    points += 1
    ...
    def calculate_posture_score(self, chair_id):
        ...
        # Calculate percentage of time with 1 sitting + 1 standing
        # For simplicity, we check if we have at least one sitting and one standing in recent frames
        # More sophisticated: track combo per frame
        combo_frames = 0
    ...
    def process_ai(self, frame):
        ...
        # --- MediaPipe Pose (every SKIP_FRAMES) ---
        pose_landmarks = None
        ...
        for obj in tracked_objects:
            ...
            if pose_landmarks:
                posture = self.classify_posture(pose_landmarks)
                if track_id not in self.posture_history:
                    self.posture_history[track_id] = []
                self.posture_history[track_id].append(posture)
                
                # Update hand activity
                prev_landmarks = None
                if track_id in self.hand_activity and self.hand_activity[track_id]:
                    # We don't store previous landmarks, so skip speed calculation for now
                    pass
                
                hand_points = self.calculate_hand_activity(pose_landmarks)
            ...
            for chair_id in range(len(self.rois)):
                if self.stable_status[chair_id]:
                    ...
                    score, breakdown = self.calculate_score(
                        ...
                        {obj.track_id: pose_landmarks for obj in tracked_objects} if pose_landmarks else {},
                        duration
                    )
