...
def get_current_summary_data():
    ...
    return {
        ...
        "valid_list": [
            serialize_session(s) for s in valid_sessions[:10]
        ],  # Limit 10 agar tidak berat
...
def get_camera_frame(request):
    if request.method == "GET":
        try:
            ...
            # LOG 1: Cek apakah frame berhasil diambil dari Redis
            print(f"[DEBUG] Redis get latest_frame: {frame_bytes is not None}")

            if frame_bytes is None:
                return JsonResponse(
                    {"status": "error", "message": "No frame available"}, status=503
                )

            # 🔥 LOG 2: Cek ukuran
            print(f"[DEBUG] Frame bytes size: {len(frame_bytes)}")

            frame_b64 = base64.b64encode(frame_bytes).decode("utf-8")

            # 🔥 LOG 3: Cek ROI
            rois = load_config()
            print(f"[DEBUG] ROIs loaded: {len(rois)} -> {rois}")

            response_data = {"status": "success", "frame": frame_b64, "rois": rois}

            # ✅ Baru di sini akses response_data
            print(f"[DEBUG] Response keys: {response_data.keys()}")
            print(f"[DEBUG] ROI in response: {response_data.get('rois', 'NOT FOUND')}")
...
def update_roi(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            rois = data.get("rois", [])
            print(f"[DEBUG] update_roi received: {len(rois)} ROIs -> {rois}")
            # Validasi format ROI
            for roi in rois:
